...
"Hey! I'm Sol, Kindly's product guide. Ask me anything about our range, opening hour
);
}
// ── Environment ──────────────────────────────────────────────────────────
...
const isStoreInfo = !isTGTG && /\b(open|close|opening|closing|hours|time|address|location
const isHiring = /\b(hiring|hire|job|jobs|vacancy|vacancies|work here|career|apply|joi
const isAbout = /\b(about kindly|who are you|what is kindly|kindly story|founded|miss
const isLoyalty = /\b(loyalty|points|reward|loyalzoo|sign up|membership|lty)\b/.test(qL
const isOrder = /\b(order|my order|order status|track|when.*deliver|delivery.*when|sh
(/\b(order|status)\b/.test(qLower) && /\d{3,6}/.test(body));
|| /^#?\d{3,6}$/.test(body.trim()); // bare order number reply
const isTGTG = /\b(tgtg|too good to go|magic bag|food bag|leftover bag)\b/.test(qLow
const isOffTopic = /\b(weather|news|sport|politics|stock|crypto|bitcoin|recipe for|how t
...
// ════════════════════════════════════════════════════════════════════════
if (isTGTG) {
solReply = "Yes! Kindly partners with Too Good To Go The best way to check today's m
fromDb = true;
} else if (/\b(phone|call|telephone|ring|contact number|speak to someone|talk to someone)
solReply = "We don't have a phone line — the best way to reach us is to pop into store
fromDb = true;
} else if (/\b(top sell|best sell|popular|most popular|bestsell|recommend|favourite|favor
// Give a curated answer from general knowledge rather than DB "first result"
solReply = "Some of our most popular picks are the Biona organic range, Clearspring pro
fromDb = false;
} else if (/(phone|call|telephone|ring|contact number|speak to someone|talk to someone|n
solReply = "We don't have a customer phone line — best way to reach us is email hello@k
fromDb = true;
} else if (/(how many (people|staff|team|employee|work)|headcount|team size|number of st
solReply = "We have 26 team members across our two Brighton stores — a passionate bunch
fromDb = true;
} else if (/(how many supplier|who (are your|do you work with)|supplier)/.test(qLower))
solReply = "We work with hundreds of suppliers — a mix of local Brighton producers and
fromDb = true;
} else if (isOffTopic) {
solReply = "I'm Sol — Kindly Brighton's product guide, so I'm best at questions about o
fromDb = false;
// ════════════════════════════════════════════════════════════════════════
// STEP 3: Store info — answer directly from Supabase (no Claude needed)
// ════════════════════════════════════════════════════════════════════════
} else if (isTGTG) {
solReply = "Yes! We're a Too Good To Go partner Our magic bags are listed on the TGT
fromDb = false;
} else if ((isStoreInfo || isHiring || isAbout || isLoyalty) && supabaseUrl && supabaseKe
try {
...
? `Our loyalty scheme is through LoyalZoo — every £1 earns points redeemable fo
: "Sign up for our loyalty scheme at the till — every £1 earns points. Ask any
} else if (isHiring) {
...
const dr = info.dyke_road_hours || 'Mon-Thu 9am-8pm, Fri-Sat 9am-7pm, Sun 10am-7
...
const wantsAddr = qLower.includes('address') || qLower.includes('where') || qLow
const wantsHours = qLower.includes('hour') || qLower.includes('open') || qLower.i
...
const co2Clean = String(co2).replace(/\s*(of CO₂|CO₂|saved|through.*)$/i, '')
const employees = info.employees ? ` We have ${info.employees} team members.`
solReply = `Kindly is Brighton's sustainable supermarket — 100% plant-based, plas
}
}
} catch(e) {
...
fromDb = true;
solReply = "I couldn't find an order matching those details. Please check you
}
}
}
} catch(e) {
...
? "I can look that up! Just send me your order number — you'll find it as #XXXX in
: "I can look that up! I need your order number (e.g. #2421 from your confirmation
}
}
// ════════════════════════════════════════════════════════════════════════
...
solReply += '\n\nFor severe allergies please check the physical label and confirm
}
}
} catch(e) {
...
if (!solReply) {
solReply = "I'm not sure about that one! Try asking about our products, opening hours,
}
} catch (err) {
console.error('WhatsApp handler error:', err.message);
solReply = "Sorry, I'm having a little trouble right now! Please try again in a moment, o
}
// ── Clean and reply ──────────────────────────────────────────────────────
...
const stopWords = new Set(['is','the','a','an','what','does','do','have','has','any','are',
...
return `You are Sol, Kindly Brighton's product guide on WhatsApp. Be warm, brief, 3-4 sente
...
`<?xml version="1.0" encoding="UTF-8"?>\n<Response><Message>${escaped}</Message></Respons
);
}
